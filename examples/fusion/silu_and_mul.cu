...
__global__ void silu_and_mul_vec4_kernel(const float *gate, const float *up,
                                         float *out, int size) {
...
int main() {
  ...
  CHECK_CUDA(cudaMemcpy(d_gate, gate.data(), size * sizeof(float),
                        cudaMemcpyHostToDevice));
  CHECK_CUDA(cudaMemcpy(d_up, up.data(), size * sizeof(float),
                        cudaMemcpyHostToDevice));
  silu_and_mul(d_gate, d_up, d_output, size);
  CHECK_CUDA(cudaGetLastError());
  CHECK_CUDA(cudaMemcpy(output.data(), d_output, size * sizeof(float),
  ...
  CHECK_CUDA(cudaMemcpy(d_weight, weight.data(), hidden_size * sizeof(float),
                        cudaMemcpyHostToDevice));
  rmsnorm_and_add(d_x, d_residual, d_weight, d_output, rows, hidden_size,
                  epsilon);
  CHECK_CUDA(cudaGetLastError());
  CHECK_CUDA(cudaMemcpy(output.data(), d_output, size * sizeof(float),
                        cudaMemcpyDeviceToHost));
  CHECK_CUDA(cudaMemcpy(residual_output.data(), d_residual,
  ...
  for (int row = 0; row < rows; ++row) {
    ...
    for (int column = 0; column < hidden_size; ++column) {
      int index = row * hidden_size + column;
      reference[index] =
          residual_reference[index] * inverse_rms * weight[column];
    }
  }
