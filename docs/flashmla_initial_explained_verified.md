# FlashMLA 首版源码核验与图解：Stream-K、mask、TMA、WGMMA 和 Swizzle

核验日期：2026-09-07。本文在工作区已有首版笔记基础上重新对照固定版本源码，保留原文件，另补充新版TMA尾行清零的layout图与地址检查。
...
| 版本 | 固定源码 | 本文用途 |
|---|---|---|
| 首个提交 | [`414a2f3`](https://github.com/deepseek-ai/FlashMLA/tree/414a2f3eedeb5ad3c4a6e89d8641e059519cacc9)，committer日期2025-02-24 | 主线：BF16、Hopper、dense MLA decode |
...
| 本次联网核对的main | [`15f13e5`](https://github.com/deepseek-ai/FlashMLA/tree/15f13e5030374295491c5ce31b02d7e63a7772c6)，2026-07-28 | 确认dense路径仍保留显式清零；不混入FP8 sparse路径 |

首版锁定的CUTLASS为`afa1772203677c5118fcd82537a9c8fefbcc7008`。本文的地址probe使用这个版本，而非只凭新版CuTe的打印结果推测旧版。

GitHub API确认该提交没有父提交；这里使用committer日期，不能把它与author日期或对外发布日混同。官方4月文章所说的previous version指向`b31bfe7`；这里按你的要求进一步回到initial commit。首版只接受BF16，FP16支持是后续提交增加的。
...
### 12.5 把新版清零的shape、stride再展开一层

![TMA尾页和逐行清零layout](assets/flashmla_tma_tail.svg)

令WG内线程号为`u`，本WG的feature包编号为`p=u%64`。每个包为8 bytes，包含4个BF16。`fill_oob_V`使用的int64 view为：

```text
实测 shape=((16,4),(8,8))
底层 stride=((1,1024),(16,128)) // int64元素单位，尚未加XOR
合并连续的token mode后，等价于：
shape=((16,4),64), stride=((1,1024),16)
p=p₀+16j                       // p₀=0..15，j=0..3
base_index=p₀+1024j+16r
```

SW128的一行128B，现在等于16个int64；一个8行atom仍占1024B。先把token方向铺满64行，再移到下一个128B feature条带，于是feature条带stride是`16×64=1024`个int64。

加上swizzle后的完整地址是：

\[
E(r,p)=1024\lfloor p/16\rfloor+16r+
((p\bmod16)\mathbin{\mathrm{XOR}}2(r\bmod8)).
\]

相对本WG半块基址，`W(r,4p)=4E(r,p)`，其中W是第7节的BF16地址公式。WG1半块基址位于整块shared K的`64×256=16384`个BF16处；它仍然正确对应原K的features256..511。

以`valid=2`为例：u0..63清r=2、4、6…62；u64..127清r=3、5、7…63。每线程31次8B写，两个WG总共覆盖`62×512`个BF16，既不漏掉无效V，也不会改动有效的r=0、1。这只是尾部路径的计数，不代表每个正常64-token block都付出这一开销。

[补充CuTe地址probe](../examples/flash_attn/flashmla_tma_clear_layout_probe.cu)逐一验证两半V与原K的地址别名，并对valid=0..64检查清零坐标恰好覆盖尾行。该probe在CPU执行，不验证GPU同步时序。

### 12.6 新版TMA的9次搬运沿哪一维切
...
截至本次核验，main与本地checkout一致，dense路径仍有[fill_oob_V](../third_party/FlashMLA/csrc/sm90/decode/dense/splitkv_mla.cuh)及同样的9条带TMA结构。4月新版按`block_idx += 2`向前处理一对KV块，亦说明首版逆序并非算法必须。[4月版TMA发射代码](https://github.com/deepseek-ai/FlashMLA/blob/c2067be3eaa0f2e98e10854c30898139d5d01d36/csrc/kernels/splitkv_mla.cu#L27-L54)

### 12.7 对“block_size=1 / 16”的评价
...
共9幅SVG均为可编辑文本和矢量图元，另提供相同尺寸PNG。前8幅沿用并核对工作区已有配图，第9幅为本次新增的TMA尾部清零图。尺寸宽度均为1200px，高度分别为890、950、1080、1050、1120、1080、1040、900、1120px。流水线图只表达先后和依赖，没有实测周期含义。

- [新增TMA图生成器](assets/flashmla_tma_tail.py)及[补充地址probe输出](assets/flashmla_tma_clear_layout_probe.txt)。
- [前8幅图形生成器](assets/flashmla_initial_figures.py)，可指定输出目录，复用本项目已有绘图与librsvg渲染helper。
...
本次重新下载首版依赖的CUTLASS `afa17722`头文件，编译并运行原始layout probe和新增的int64清零probe；重新运行CPU数值检查。9幅图均查看完整PNG，并检查SVG结构与链接。新增图生成器以默认目录和临时目录各运行一次，核对结果可重复。
